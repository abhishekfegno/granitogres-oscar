...
def product_list(request, category='all', **kwargs):
    ...
    if _product_range:
        ...
        queryset = product_range.all_products().filter(structure__in=['standalone', 'child'],
                                                       is_public=True, stockrecords__isnull=False)
        ...
        queryset = offer_banner_object.products().filter(structure__in=['standalone', 'child'],
                                                         is_public=True, stockrecords__isnull=False)
    elif category != 'all':
    ...
    def _inner():
        ...
        # pn_obj = pn.paginate_queryset(queryset, request)
        # ser_data = serializer_class(pn_obj, many=True, context={'request': request}).data
        # return OrderedDict([
        #     ('count', pn_obj.page.paginator.count),
        #     ('next', pn_obj.get_next_link()),
        #     ('previous', pn_obj.get_previous_link()),
        #     ('results', ser_data)
        # ])
        rc = None
        # if category != 'all':
        #     rc = recommended_class(queryset)
        return list_api_formatter(request, paginator=paginator, page_obj=page_obj, results=product_data, product_class=rc)
